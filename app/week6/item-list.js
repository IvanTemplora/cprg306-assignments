    ...
    if(sortBy === "name"){
      ...
      return(
        <main>
            <div>
                <button
                className={`border border-whi   te p-2 m-2 ${sortBy === "name" ? 'bg-white text-black' : 'bg-black'}`}
                onClick={sortByName}>Sort By Name</button>
                <button
                className={`border border-white p-2 m-2 ${sortBy === "category" ? 'bg-white text-black' : 'bg-black'}`}
                onClick={sortByCategory}>Sort By Category</button>
            </div>
            <div>
                {sortedList.map((item) => (
